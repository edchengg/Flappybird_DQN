...
class DQN_AI():
    ...
    def __init__(self, num_actions, mode, dueling_Q = False):
        ...
        if dueling_Q:
            self.model = self.Q_Duel_CNN_Setup()
        else:
    ...
    def Q_Duel_CNN_Setup(self):

        # Prepare Dueling Convolutional Neural Network for Q-Learning
        inputs = Input(shape=self.input_shape)
        # CNN layer_1
        net = Conv2D(16, kernel_size = (8, 8), activation = 'relu')(inputs)
        # CNN layer_2
        net = Conv2D(32, kernel_size = (4, 4), activation = 'relu')(net)
        # CNN layer_3
        ...
        # FC layer_2
        value = Dense(1)(value)
        # Combine Advantage layer and Q value layer
        policy = merge([advt, value], mode = lambda x: x[0] -K.mean(x[0])+x[1], output_shape=(self.num_actions,))
        ...
        model = Model(input=[inputs], output=[policy])
        # Optimizer
        optimizer = keras.optimizers.Adam(lr=LEARNING_RATE)
        # Compile the model
        model.compile(loss = keras.losses.mean_squared_error, optimizer = optimizer)
